import{n as e}from"./chunk-zsgVPwQN.js";import{X as t,Y as n,at as r,er as i,it as a,ot as o,rt as s,st as c,tr as l,zh as u}from"./iframe-DqFZvJJc.js";var d,f,p=e((()=>{d=u(),f=({height:e=40})=>(0,d.jsxs)(`svg`,{fill:`none`,xmlns:`http://www.w3.org/2000/svg`,viewBox:`0 0 225 40`,height:e,children:[(0,d.jsx)(`path`,{d:`M157.947 12.986c0 .569.115 1.041.347 1.417.231.367.54.67.925.912.396.24.849.438 1.36.593.52.154 1.06.298 1.619.433.753.183 1.49.386 2.213.608.733.221 1.384.53 1.953.925a4.352 4.352 0 0 1 1.373 1.519c.357.627.536 1.432.536 2.415 0 1.003-.188 1.87-.564 2.603a4.878 4.878 0 0 1-1.548 1.793c-.656.463-1.441.805-2.357 1.027-.916.232-1.919.347-3.008.347-.502 0-1.017-.033-1.548-.1-.52-.059-1.022-.14-1.504-.247a11.65 11.65 0 0 1-1.287-.361 4.923 4.923 0 0 1-.911-.405V24.31c.347.174.738.333 1.171.477.434.145.883.27 1.345.376.463.097.931.174 1.403.232.473.057.916.086 1.331.086.675 0 1.316-.058 1.923-.173a5.033 5.033 0 0 0 1.62-.579c.463-.27.829-.636 1.099-1.099.27-.472.405-1.06.405-1.764 0-.579-.121-1.056-.362-1.432a2.777 2.777 0 0 0-.94-.925 5.43 5.43 0 0 0-1.374-.608 27.818 27.818 0 0 0-1.619-.434 39.125 39.125 0 0 1-2.213-.578 7.22 7.22 0 0 1-1.923-.911 4.478 4.478 0 0 1-1.36-1.49c-.337-.607-.506-1.383-.506-2.328 0-.984.173-1.827.521-2.531a4.906 4.906 0 0 1 1.417-1.765 6.006 6.006 0 0 1 2.097-1.026 9.545 9.545 0 0 1 2.56-.333c1.031 0 1.991.101 2.878.304a13.37 13.37 0 0 1 2.458.766v2.184a16.7 16.7 0 0 0-2.487-.824 11.733 11.733 0 0 0-2.734-.333c-.732 0-1.369.082-1.909.246-.54.154-.988.38-1.345.68-.356.289-.621.646-.795 1.07-.174.414-.26.877-.26 1.388ZM181.965 20.362c0-.849-.106-1.6-.319-2.256-.202-.666-.491-1.225-.867-1.678a3.516 3.516 0 0 0-1.316-1.041 3.883 3.883 0 0 0-1.678-.362c-.607 0-1.167.12-1.677.362a3.635 3.635 0 0 0-1.331 1.041c-.366.453-.656 1.012-.868 1.678-.202.655-.303 1.407-.303 2.256 0 .839.101 1.59.303 2.256.212.655.506 1.21.882 1.663.376.453.82.8 1.331 1.041.511.232 1.07.347 1.678.347.607 0 1.161-.115 1.663-.347a3.72 3.72 0 0 0 1.316-1.041c.376-.453.665-1.008.867-1.663.213-.665.319-1.417.319-2.256Zm2.342 0c0 1.08-.154 2.063-.462 2.95-.309.887-.748 1.649-1.316 2.285a5.781 5.781 0 0 1-2.039 1.46c-.801.348-1.697.521-2.69.521-1.022 0-1.938-.173-2.748-.52a5.921 5.921 0 0 1-2.054-1.461c-.559-.636-.988-1.398-1.287-2.285-.299-.887-.448-1.87-.448-2.95 0-1.09.154-2.078.463-2.965.308-.887.742-1.649 1.301-2.285a5.826 5.826 0 0 1 2.054-1.475c.81-.347 1.711-.52 2.704-.52 1.012 0 1.924.173 2.733.52a5.66 5.66 0 0 1 2.054 1.475c.559.636.988 1.398 1.287 2.285.299.887.448 1.875.448 2.965ZM193.603 13.116c.665 0 1.302.073 1.909.217.608.135 1.128.328 1.562.579v1.865a12.518 12.518 0 0 0-1.678-.52 7.045 7.045 0 0 0-1.561-.174c-.56 0-1.114.092-1.664.275a3.823 3.823 0 0 0-1.475.911c-.434.415-.786.969-1.055 1.663-.261.685-.391 1.538-.391 2.56 0 .742.096 1.427.289 2.054.203.626.497 1.171.882 1.634.396.453.887.81 1.476 1.07.588.25 1.267.376 2.039.376.559 0 1.123-.058 1.692-.174a10.074 10.074 0 0 0 1.663-.52v1.865a3.898 3.898 0 0 1-.651.29c-.26.096-.549.178-.868.245a7.877 7.877 0 0 1-1.012.174 7.981 7.981 0 0 1-1.07.072c-.926 0-1.803-.14-2.632-.419a5.89 5.89 0 0 1-2.155-1.287c-.617-.579-1.104-1.311-1.461-2.198-.356-.887-.535-1.938-.535-3.153 0-.906.092-1.721.275-2.444.193-.723.453-1.36.781-1.91a5.928 5.928 0 0 1 1.142-1.402 6.524 6.524 0 0 1 1.403-.94 6.15 6.15 0 0 1 1.548-.535 7.253 7.253 0 0 1 1.547-.174ZM200.686 27.217v-13.74h2.242v13.74h-2.242Zm-.058-16.935V7.78h2.358v2.502h-2.358ZM215.318 25.698h-.058a4.682 4.682 0 0 1-.694.694 5.502 5.502 0 0 1-.969.608 6.271 6.271 0 0 1-1.2.42 5.84 5.84 0 0 1-1.389.158 5.354 5.354 0 0 1-1.836-.303 4.042 4.042 0 0 1-1.432-.854 4.057 4.057 0 0 1-.926-1.402c-.221-.55-.332-1.172-.332-1.866 0-.685.12-1.297.361-1.837.251-.54.598-.998 1.042-1.374a4.68 4.68 0 0 1 1.59-.867 6.704 6.704 0 0 1 2.069-.304c.713.01 1.373.072 1.981.188.607.116 1.147.26 1.62.434h.057v-.955a6.15 6.15 0 0 0-.072-.997 2.541 2.541 0 0 0-.246-.767c-.25-.463-.65-.844-1.2-1.143-.55-.308-1.292-.462-2.227-.462-.694 0-1.35.067-1.967.202-.607.125-1.215.304-1.822.535v-1.88c.231-.116.501-.222.81-.318.318-.106.65-.193.997-.26a9.388 9.388 0 0 1 1.1-.174c.385-.038.771-.058 1.157-.058 1.388 0 2.521.246 3.398.738.887.492 1.523 1.142 1.909 1.952.145.309.246.651.304 1.027.067.366.101.771.101 1.215v9.169h-1.88l-.246-1.519Zm-.116-4.512a17.448 17.448 0 0 0-1.475-.332 11.13 11.13 0 0 0-1.88-.174c-.954 0-1.701.202-2.241.607-.54.405-.81 1.027-.81 1.866 0 .424.067.795.202 1.114.135.318.318.583.55.795.241.202.52.357.839.463.327.096.675.144 1.041.144.482 0 .93-.067 1.345-.202a5.82 5.82 0 0 0 1.099-.506c.328-.193.603-.386.824-.579.232-.193.4-.342.506-.448v-2.748ZM221.794 27.217V6.276h2.241v20.94h-2.241Z`,fill:`#000`}),(0,d.jsx)(`path`,{d:`M20 40c11.047 0 20.002-8.955 20.002-20C40.002 8.952 31.047 0 20 0 8.955 0 0 8.955 0 20c0 11.047 8.955 20 20 20Z`,fill:`#069E08`}),(0,d.jsx)(`path`,{d:`M20.993 16.642V36.03l10-19.39h-10ZM18.97 23.321V3.97L9.008 23.321h9.962Z`,fill:`#fff`}),(0,d.jsx)(`path`,{d:`M51.642 33.206c-.574-.878-1.107-1.755-1.642-2.595 2.825-1.718 3.779-3.09 3.779-5.687v-15h-3.32V7.061h7.061v17.1c0 4.35-1.259 6.793-5.879 9.044ZM81.223 22.978c0 1.45 1.031 1.602 1.719 1.602.687 0 1.68-.23 2.442-.457v2.671c-1.07.344-2.176.61-3.703.61-1.832 0-3.97-.686-3.97-3.893v-7.863h-1.945v-2.71h1.946V8.93h3.511v4.008h4.427v2.71h-4.427v7.329ZM88.552 34.542V12.9h3.359v1.298c1.336-1.031 2.825-1.68 4.657-1.68 3.168 0 5.687 2.214 5.687 6.985 0 4.734-2.747 7.863-7.29 7.863-1.106 0-1.985-.152-2.901-.343v7.48h-3.512v.04ZM95.65 15.42c-1.031 0-2.328.496-3.55 1.565v7.367c.762.153 1.565.267 2.633.267 2.48 0 3.894-1.566 3.894-4.848 0-3.015-1.031-4.351-2.977-4.351ZM116.071 27.061h-3.282v-1.565h-.077c-1.145.878-2.558 1.832-4.657 1.832-1.832 0-3.817-1.336-3.817-4.047 0-3.625 3.091-4.313 5.267-4.618l3.091-.419v-.419c0-1.91-.763-2.52-2.558-2.52-.878 0-2.938.267-4.618.954l-.305-2.824c1.527-.535 3.626-.915 5.382-.915 3.435 0 5.649 1.374 5.649 5.458v9.083h-.075Zm-3.512-6.602-2.902.457c-.878.114-1.793.649-1.793 1.946 0 1.145.649 1.794 1.602 1.794 1.031 0 2.138-.61 3.091-1.298v-2.9h.002ZM130.575 26.604c-1.45.496-2.747.8-4.39.8-5.267 0-7.367-3.015-7.367-7.403 0-4.618 2.902-7.481 7.595-7.481 1.755 0 2.825.305 4.008.687v2.977c-1.031-.382-2.519-.801-3.969-.801-2.138 0-3.97 1.145-3.97 4.427 0 3.626 1.832 4.733 4.161 4.733 1.106 0 2.328-.23 3.969-.878v2.939h-.037ZM137.216 19.008c.306-.343.535-.687 4.962-6.07h4.58l-5.726 6.72 6.259 7.442h-4.58l-5.458-6.719V27.1h-3.51V7.062h3.512v11.946h-.039ZM73.13 26.603c-1.831.574-3.397.802-5.229.802-4.504 0-7.29-2.252-7.29-7.52 0-3.855 2.367-7.367 6.908-7.367 4.504 0 6.07 3.13 6.07 6.107 0 .992-.078 1.527-.115 2.099h-9.083c.077 3.09 1.832 3.816 4.465 3.816 1.45 0 2.748-.343 4.238-.878v2.939h.037v.002Zm-3.204-8.206c0-1.718-.573-3.207-2.443-3.207-1.755 0-2.824 1.259-3.054 3.207h5.497Z`,fill:`#000`})]}),f.__docgenInfo={description:``,methods:[],displayName:`Logo`,props:{height:{defaultValue:{value:`40`,computed:!1},required:!1}}}})),m,h,g,_,v,y,b,x,S,C,w,T,E;e((()=>{l(),t(),c(),p(),m=u(),h={title:`JS Packages/Components/Pricing Table`,component:s,subcomponents:{PricingTableColumn:a,PricingTableHeader:r,PricingTableItem:o}},g=e=>(0,m.jsxs)(s,{...e,children:[(0,m.jsxs)(a,{primary:!0,children:[(0,m.jsxs)(r,{children:[(0,m.jsx)(n,{price:9.95,offPrice:4.98,legend:`/month, billed yearly`,currency:`USD`,hideDiscountLabel:e?.hideDiscountLabel}),(0,m.jsx)(i,{fullWidth:!0,children:`Get Premium`})]}),(0,m.jsx)(o,{isIncluded:!0,label:(0,m.jsx)(`strong`,{children:`Up to 1000`})}),(0,m.jsx)(o,{isIncluded:!0,tooltipInfo:`This is an info`}),(0,m.jsx)(o,{isIncluded:!0,tooltipInfo:`This is an info with title`,tooltipTitle:`Small title`}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0})]}),(0,m.jsxs)(a,{children:[(0,m.jsxs)(r,{children:[(0,m.jsx)(n,{price:0,legend:`Free forever`,currency:`USD`,hidePriceFraction:!0}),(0,m.jsx)(i,{fullWidth:!0,variant:`secondary`,children:`Start for free`})]}),(0,m.jsx)(o,{isIncluded:!0,label:`Up to 300`}),(0,m.jsx)(o,{isIncluded:!1,label:`This is not included`,tooltipInfo:`This has a tooltip, so its overwrites the default info on small screens`}),(0,m.jsx)(o,{isIncluded:!1}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0})]})]}),_=[{name:`Priority support`},{name:`Schedule posting`},{name:`Share to Facebook, Instagram, LinkedIn`},{name:`Customize publications`},{name:`Recycle content`,tooltipInfo:`Repurpose, reuse or republish already published content.`},{name:`Upload custom images with your posts`},{name:`Upload videos with your posts`},{name:`Automatically generate images for posts`,tooltipInfo:`Automatically create custom images, saving you hours of tedious work.`},{name:`Multi-image sharing`,tooltipTitle:`Coming soon`,tooltipInfo:`Share multiple images at once on social media platforms.`}],v={title:`Buy premium, or start for free`,items:[{name:`Feature A with limit`,tooltipInfo:`Default info for Feature A`},{name:`Feature B`,tooltipInfo:`Default info for Feature B`},{name:`Feature C with a longer title that will span multiple lines`,tooltipInfo:`Default info for Feature C`,tooltipTitle:`Title for C`},{name:`Feature D`,tooltipInfo:`Default info for Feature D`,tooltipTitle:`Title for D`},{name:`Feature E`}],hideDiscountLabel:!1},y=g.bind({}),y.args=v,b=e=>(0,m.jsxs)(s,{...e,children:[(0,m.jsxs)(a,{primary:!0,children:[(0,m.jsxs)(r,{title:`Social`,children:[(0,m.jsx)(n,{price:24.95,offPrice:12.48,legend:`/month, billed yearly`,currency:`USD`,hidePriceFraction:!0,variant:`simple`}),(0,m.jsx)(i,{fullWidth:!0,children:`Get Social`})]}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!1,isComingSoon:!0})]}),(0,m.jsxs)(a,{children:[(0,m.jsxs)(r,{title:`Free`,children:[(0,m.jsx)(n,{price:0,legend:`Free forever`,currency:`USD`,hidePriceFraction:!0,variant:`simple`}),(0,m.jsx)(i,{fullWidth:!0,variant:`secondary`,children:`Start for free`})]}),(0,m.jsx)(o,{isIncluded:!1}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!1}),(0,m.jsx)(o,{isIncluded:!1}),(0,m.jsx)(o,{isIncluded:!1}),(0,m.jsx)(o,{isIncluded:!1})]})]}),x=b.bind({}),x.args={title:`Write once, post everywhere`,headerLogo:(0,m.jsx)(f,{height:32}),items:_,showIntroOfferDisclaimer:!1},S=e=>(0,m.jsxs)(s,{...e,children:[(0,m.jsxs)(a,{children:[(0,m.jsxs)(r,{children:[(0,m.jsx)(n,{price:0,currency:`USD`,hidePriceFraction:!0}),(0,m.jsx)(i,{fullWidth:!0,variant:`secondary`,children:`Free`})]}),(0,m.jsx)(o,{isIncluded:!0,label:`Basic version`}),(0,m.jsx)(o,{isIncluded:!1}),(0,m.jsx)(o,{isIncluded:!0,label:`5 per month`})]}),(0,m.jsxs)(a,{primary:!0,children:[(0,m.jsxs)(r,{children:[(0,m.jsx)(n,{price:15.95,currency:`USD`,hidePriceFraction:!0,legend:`/month`}),(0,m.jsx)(i,{fullWidth:!0,children:`Pro`})]}),(0,m.jsx)(o,{isIncluded:!0,label:`Full version`}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0,label:`Unlimited`})]})]}),C=S.bind({}),C.args={title:`Feature comparison`,items:[{name:`Custom feature`},{name:`Special feature`},{name:`Limited feature`}],showIntroOfferDisclaimer:!1},w=e=>(0,m.jsxs)(s,{...e,children:[(0,m.jsxs)(a,{children:[(0,m.jsxs)(r,{title:`Starter`,children:[(0,m.jsx)(n,{price:0,legend:`Free forever`,currency:`USD`,hidePriceFraction:!0,variant:`simple`}),(0,m.jsx)(i,{fullWidth:!0,variant:`secondary`,children:`Start Free`})]}),(0,m.jsx)(o,{isIncluded:!0,label:`Up to 5 sites`}),(0,m.jsx)(o,{isIncluded:!0,label:`Basic features`}),(0,m.jsx)(o,{isIncluded:!1}),(0,m.jsx)(o,{isIncluded:!1}),(0,m.jsx)(o,{isIncluded:!1}),(0,m.jsx)(o,{isIncluded:!1})]}),(0,m.jsxs)(a,{primary:!0,children:[(0,m.jsxs)(r,{title:`Professional`,children:[(0,m.jsx)(n,{price:24.95,offPrice:12.48,legend:`/month, billed yearly`,currency:`USD`,hidePriceFraction:!0,variant:`simple`}),(0,m.jsx)(i,{fullWidth:!0,children:`Get Professional`})]}),(0,m.jsx)(o,{isIncluded:!0,label:`Up to 50 sites`}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0,label:`Advanced analytics`}),(0,m.jsx)(o,{isIncluded:!0,label:`Priority support`}),(0,m.jsx)(o,{isIncluded:!0,label:`Custom integrations`}),(0,m.jsx)(o,{isIncluded:!1})]}),(0,m.jsxs)(a,{children:[(0,m.jsxs)(r,{title:`Enterprise`,children:[(0,m.jsx)(n,{price:99.95,offPrice:79.96,legend:`/month, billed yearly`,currency:`USD`,hidePriceFraction:!0,variant:`simple`}),(0,m.jsx)(i,{fullWidth:!0,variant:`secondary`,children:`Contact Sales`})]}),(0,m.jsx)(o,{isIncluded:!0,label:`Unlimited sites`}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0}),(0,m.jsx)(o,{isIncluded:!0,label:`White-label solution`})]})]}),T=w.bind({}),T.args={title:`Choose your plan`,items:[{name:`Sites included`},{name:`Essential features`},{name:`Analytics & reporting`},{name:`Customer support`},{name:`API access`},{name:`Enterprise features`}],headerLogo:(0,m.jsx)(f,{height:32}),showIntroOfferDisclaimer:!1},y.parameters={...y.parameters,docs:{...y.parameters?.docs,source:{originalSource:`args => {
  return <PricingTable {...args}>
            <PricingTableColumn primary>
                <PricingTableHeader>
                    <ProductPrice price={9.95} offPrice={4.98} legend="/month, billed yearly" currency="USD" hideDiscountLabel={args?.hideDiscountLabel} />
                    <Button fullWidth>Get Premium</Button>
                </PricingTableHeader>
                <PricingTableItem isIncluded={true} label={<strong>Up to 1000</strong>} />
                <PricingTableItem isIncluded={true} tooltipInfo={'This is an info'} />
                <PricingTableItem isIncluded={true} tooltipInfo={'This is an info with title'} tooltipTitle={'Small title'} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
            </PricingTableColumn>
            <PricingTableColumn>
                <PricingTableHeader>
                    <ProductPrice price={0} legend="Free forever" currency="USD" hidePriceFraction />
                    <Button fullWidth variant="secondary">
                        Start for free
                    </Button>
                </PricingTableHeader>
                <PricingTableItem isIncluded={true} label="Up to 300" />
                <PricingTableItem isIncluded={false} label="This is not included" tooltipInfo="This has a tooltip, so its overwrites the default info on small screens" />
                <PricingTableItem isIncluded={false} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
            </PricingTableColumn>
        </PricingTable>;
}`,...y.parameters?.docs?.source}}},x.parameters={...x.parameters,docs:{...x.parameters?.docs,source:{originalSource:`args => {
  return <PricingTable {...args}>
            <PricingTableColumn primary>
                <PricingTableHeader title="Social">
                    <ProductPrice price={24.95} offPrice={12.48} legend="/month, billed yearly" currency="USD" hidePriceFraction variant="simple" />
                    <Button fullWidth>Get Social</Button>
                </PricingTableHeader>
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={false} isComingSoon={true} />
            </PricingTableColumn>
            <PricingTableColumn>
                <PricingTableHeader title="Free">
                    <ProductPrice price={0} legend="Free forever" currency="USD" hidePriceFraction variant="simple" />
                    <Button fullWidth variant="secondary">
                        Start for free
                    </Button>
                </PricingTableHeader>
                <PricingTableItem isIncluded={false} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={false} />
                <PricingTableItem isIncluded={false} />
                <PricingTableItem isIncluded={false} />
                <PricingTableItem isIncluded={false} />
            </PricingTableColumn>
        </PricingTable>;
}`,...x.parameters?.docs?.source}}},C.parameters={...C.parameters,docs:{...C.parameters?.docs,source:{originalSource:`args => {
  return <PricingTable {...args}>
            <PricingTableColumn>
                <PricingTableHeader>
                    <ProductPrice price={0} currency="USD" hidePriceFraction />
                    <Button fullWidth variant="secondary">
                        Free
                    </Button>
                </PricingTableHeader>
                <PricingTableItem isIncluded={true} label="Basic version" />
                <PricingTableItem isIncluded={false} />
                <PricingTableItem isIncluded={true} label="5 per month" />
            </PricingTableColumn>
            <PricingTableColumn primary>
                <PricingTableHeader>
                    <ProductPrice price={15.95} currency="USD" hidePriceFraction legend="/month" />
                    <Button fullWidth>Pro</Button>
                </PricingTableHeader>
                <PricingTableItem isIncluded={true} label="Full version" />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} label="Unlimited" />
            </PricingTableColumn>
        </PricingTable>;
}`,...C.parameters?.docs?.source}}},T.parameters={...T.parameters,docs:{...T.parameters?.docs,source:{originalSource:`args => {
  return <PricingTable {...args}>
            <PricingTableColumn>
                <PricingTableHeader title="Starter">
                    <ProductPrice price={0} legend="Free forever" currency="USD" hidePriceFraction variant="simple" />
                    <Button fullWidth variant="secondary">
                        Start Free
                    </Button>
                </PricingTableHeader>
                <PricingTableItem isIncluded={true} label="Up to 5 sites" />
                <PricingTableItem isIncluded={true} label="Basic features" />
                <PricingTableItem isIncluded={false} />
                <PricingTableItem isIncluded={false} />
                <PricingTableItem isIncluded={false} />
                <PricingTableItem isIncluded={false} />
            </PricingTableColumn>
            <PricingTableColumn primary>
                <PricingTableHeader title="Professional">
                    <ProductPrice price={24.95} offPrice={12.48} legend="/month, billed yearly" currency="USD" hidePriceFraction variant="simple" />
                    <Button fullWidth>Get Professional</Button>
                </PricingTableHeader>
                <PricingTableItem isIncluded={true} label="Up to 50 sites" />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} label="Advanced analytics" />
                <PricingTableItem isIncluded={true} label="Priority support" />
                <PricingTableItem isIncluded={true} label="Custom integrations" />
                <PricingTableItem isIncluded={false} />
            </PricingTableColumn>
            <PricingTableColumn>
                <PricingTableHeader title="Enterprise">
                    <ProductPrice price={99.95} offPrice={79.96} legend="/month, billed yearly" currency="USD" hidePriceFraction variant="simple" />
                    <Button fullWidth variant="secondary">
                        Contact Sales
                    </Button>
                </PricingTableHeader>
                <PricingTableItem isIncluded={true} label="Unlimited sites" />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} />
                <PricingTableItem isIncluded={true} label="White-label solution" />
            </PricingTableColumn>
        </PricingTable>;
}`,...T.parameters?.docs?.source}}},E=[`_default`,`WithLogo`,`WithCustomLabels`,`ThreeColumns`]}))();export{T as ThreeColumns,C as WithCustomLabels,x as WithLogo,E as __namedExportsOrder,y as _default,h as default};